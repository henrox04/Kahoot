...
Your tests should be *black box* unit tests:
  ...
  * For iteration 1, you should *not* be importing the `data` object itself or directly accessing it via the `get` or `set` functions from `src/dataStore.ts` inside your tests.
...
* Files used for imports are appended with `.js` e.g. `import { clearV1 } from './other';`
